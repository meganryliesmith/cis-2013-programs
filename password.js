...
var passwordCheckOld = function () 
{
 ...
 var i = 0;
 var intError;
 while (i <= arrayBadPasswords.length){ //loop to test if input matches data in array
   if(stringPassword == arrayBadPasswords[i]){ //tests to see if inputed password is in bad password array
      intError = 1;
   } else if (stringPassword.length < 8){ //tests to see if inputed password is less than 8 characters
      intError = 2;
   }
   i++;
 }
 if(intError == 1){ // if password matches a password in bad password array, alert user with message "This is a bad password." 
   stringOutput = "This is a bad password.";
 } else if(intError == 2){ //if password is less than 8 characters, alert user with message "This password doesn't have enough characters."
   stringOutput = "This password doesn't have enough characters.";
 } else { //if no error is detected (all else), alert user with message "This is a good password."
   stringOutput = "This is a good password.";
 }
 $("output").value = stringOutput; //sends output to the DOM
};
